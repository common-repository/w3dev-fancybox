	...
	*/
	...
	$.extend(FancyBox.prototype, {
		...
		getFitPos: function (slide) {
			...
			maxWidth =
				parseInt(self.$refs.stage.width(), 10) - (margin.left + margin.right);
			maxHeight =
				parseInt(self.$refs.stage.height(), 10) - (margin.top + margin.bottom);
		...
		updateSlide: function (slide, duration) {
			...
			self.$refs.toolbar.toggleClass(
				...
				slide.$slide.get(0).scrollHeight > slide.$slide.get(0).clientHeight
			);
		...
		canPan: function () {
			...
			if (
				...
				rez =
					Math.abs($content.width() - rez.width) > 1 ||
					Math.abs($content.height() - rez.height) > 1;
			}
		...
		updateControls: function (force) {
			...
			$container
				...
				.toggleClass('disabled', !current.opts.loop && index <= 0);
			$container
	...
	$D.on('click.fb-start', '[data-trigger]', function (e) {
		_run(e, {
			$target: $(
				'[data-fancybox="' + $(e.currentTarget).attr('data-trigger') + '"]'
...
(function (window, document, $) {
	...
	Guestures.prototype.ontouchstart = function (e) {
		...
		self.sliderStartPos =
			self.sliderLastPos || $.fancybox.getTranslate(current.$slide);
